    ...
    if gone:
        err('%s: нема гліфа в шрифті: %s у %r'
            % (where, ' '.join('%s U+%04X' % (c, ord(c)) for c in sorted(gone)),
               text[:60]))
    # третє, і його не видно ні в cp1251, ні в шрифті: ’ і ‘ там Є, тож
    # проходять обидві перевірки, - але апостроф у корпусі мусить бути один
    for c in '’‘':
        if c in text:
            err('%s: апостроф %s U+%04X — у корпусі лише ASCII \' (%r)'
                % (where, c, ord(c), text[:60]))
