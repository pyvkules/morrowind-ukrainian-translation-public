# -*- coding: utf-8 -*-
"""Транслітерація власних імен (NPC) англ.->укр., з перекладом епітетів і ролей.

Імена в Morrowind змішані:
  * власні особові імена (Raril Giral, Collatinus Clanler) — транслітеруємо;
  * описові епітети (Whitebeard, Wave-Breaker, the Smith) — ПЕРЕКЛАДАЄМО;
  * загальні ролі (Bowman, Rogue Necromancer) — ПЕРЕКЛАДАЄМО.

Це чорновий двигун: вихід треба вичитувати. Усталені лорні імена й будь-що
криве кладемо в overrides (tools/items/npc_overrides.json) — воно має пріоритет.

    py translit.py            # показати зразок
    py translit.py --apply    # записати uk_npc.json
"""
import io
import json
import os
import re
import sys

sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8', errors='replace')
HERE = os.path.dirname(os.path.abspath(__file__))

# слова, які ПЕРЕКЛАДАЮТЬСЯ, а не транслітеруються (епітети, ролі, титули).
# Це лише РИШТУВАННЯ: остаточний варіант кожного пакета вичитується вручну.
WORDS = {
    'the': '', 'of': '', 'gro': 'ґро', 'gra': 'ґра',
    'whitebeard': 'Білобородий', 'fine-hair': 'Тонковолосий',
    'wave-breaker': 'Хвилелам', 'smith': 'Коваль', 'bowman': 'Лучник',
    'necromancer': 'Некромант', 'rogue': 'Відступник', 'guard': 'Вартовий',
    'trader': 'Торговець', 'bonebiter': 'Костогриз', 'fire-eye': 'Вогнеокий',
    'strong': 'Сильний', 'redoran': 'Редоран', 'wise': 'Мудрий',
    'blackheart': 'Чорносердий', 'bloodward': 'Кровострах', 'old': 'Старий',
    'young': 'Молодий', 'black': 'Чорний', 'white': 'Білий', 'red': 'Рудий',
    'the-cunning': 'Хитрий', 'cunning': 'Хитрий', 'brave': 'Хоробрий',
    'swift': 'Прудкий', 'quick': 'Прудкий', 'grey': 'Сивий', 'gray': 'Сивий',
    'ill-lit': '', 'necromancer,': 'Некромант', 'wild': 'Дикий',
    'jeweler': 'Ювелір', 'jeweller': 'Ювелір', 'admiral': 'Адмірал',
    'captain': 'Капітан', 'sergeant': 'Сержант', 'general': 'Генерал',
    'tall': 'Високий', 'one-eye': 'Одноокий', 'one-hand': 'Однорукий',
    'half-troll': 'Напівтроль', 'the-tongueless': 'Безъязикий',
    'healer': 'Цілитель', 'the-healer': 'Цілитель', 'wanderer': 'Мандрівник',
    'the-wanderer': 'Мандрівник', 'the-mad': 'Божевільний', 'mad': 'Божевільний',
    'the-fair': 'Прекрасний', 'fair': 'Прекрасний', 'lord': 'Лорд',
    'lady': 'Пані', 'king': 'Король', 'queen': 'Королева', 'saint': 'Святий',
}

# Прізвиська нордів та описові імена аргоніан. Правило те саме, що й скрізь:
# власну назву транслітеруємо, ОПИСОВЕ перекладаємо. Доти сюди не заглядали,
# і виходило «Хлоґґар Блуді», «Свімс-Ін-Свеллс», «Тарстен Геарт-Фанґ».
#
# Аргонійські та ешлендерські імена з дефісом, що НЕ складаються з англійських
# слів (Ashur-Dan, Chanil-Lee, Sul-Matuul, Heem-La), сюди не входять - вони
# лишаються транслітерованими, і це правильно.
#
# Рід. Словник плаский, тож прикметникові прізвиська тут у чоловічому роді.
# Жінкам («Erna the Quiet», «Fjori the Stout») ставимо форму вручну
# в npc_overrides.json. Іменникові складанки («Снігова-Пісня», «Довга-Нога»)
# роду не потребують і годяться всім.
WORDS.update({
    # норди й імперці: «X the Y»
    'bloody': 'Кривавий', 'bold': 'Сміливий', 'braggart': 'Хвалько',
    'candle': 'Свічка', 'clerk': 'Писар', 'craven': 'Боягуз',
    'crow': 'Ворон', 'dreamer': 'Сновида', 'easterner': 'Східняк',
    'flayer': 'Шкуродер', 'gentle': 'Лагідний', 'halt': 'Кульгавий',
    'harrier': 'Гонитель', 'lean': 'Худий', 'liar': 'Брехун',
    'long': 'Довгий', 'magnificent': 'Величний', 'mumbling': 'Бурмило',
    'outlaw': 'Розбійник', 'peacock': 'Павич', 'quiet': 'Тихий',
    'rascal': 'Пройдисвіт', 'raven': 'Крук', 'roarer': 'Ревун',
    'seal': 'Тюлень', 'stablemaster': 'Стайничий', 'stout': 'Кремезний',
    'scrib': 'Скриб',   # істота, а не транслітерація: див. creature_overrides
    'strange': 'Дивний', 'sweltering': 'Упрілий', 'undying': 'Невмирущий',
    'unending': 'Нескінченний', 'unworthy': 'Негідний', 'bow': 'Лук',
    'chambermaid': 'Покоївка',
    # складанки з дефісом: норди
    'blue-tooth': 'Синьозубий', 'cold-hands': 'Холодні-Руки',
    'elf-hewer': 'Ельфоруб', 'flat-foot': 'Плоскостопий',
    'fork-beard': 'Вилобородий', 'gray-wave': 'Сіра-Хвиля',
    'hairy-breeks': 'Волохаті-Штани', 'hard-heart': 'Тверде-Серце',
    'heart-fang': 'Серце-Ікло', 'hearth-healer': 'Хатня-Цілителька',
    'home-wrecker': 'Руйнівник-Домівок', 'horse-mouth': 'Кінський-Рот',
    'ice-mane': 'Крижана-Грива', 'long-leg': 'Довга-Нога',
    'long-tooth': 'Довгий-Зуб', 'red-spear': 'Червоний-Спис',
    'red-tooth': 'Червоний-Зуб', 'snow-song': 'Снігова-Пісня',
    'storm-bane': 'Погибель-Бурі', 'swift-sailer': 'Прудка-Мореплавниця',
    'wind-eye': 'Око-Вітру', 'wine-sot': 'Винний-Пияк',
    'wolf-runner': 'Вовкобіг', 'bold-lute': 'Зухвала-Лютня',
    'deep-raed': 'Глибокодумний', 'new-shoes': 'Нові-Чоботи',
    'nine-toes': "Дев'ять-Пальців", 'rainy-man': 'Дощовий-Чоловік',
    'axe-wife': 'Сокирниця', 'crazy-legs': 'Скажені-Ноги',
    'sugar-lips': 'Цукрові-Губи', 'eye-fly': 'Око-Муха',
    'fly-breath': 'Мушачий-Подих', 'hard-mouth': 'Твердоуст',
    # складанки з дефісом: аргоніани (описове ім'я - їхній звичай)
    'also-he-washes': "Він-Ще-Й-Миється",
    'basks-in-the-sun': 'Гріється-На-Сонці',
    'blood-in-his-eyes': 'Кров-В-Очах', 'dreaded-water': 'Страшна-Вода',
    'fine-mouth': 'Красноуст', 'grey-throat': 'Сіре-Горло',
    'hides-his-foot': 'Ховає-Свою-Ногу', 'high-heart': 'Високе-Серце',
    'high-pockets': 'Високі-Кишені', 'lurks-within-depths': 'Чигає-В-Глибині',
    'morning-star-steals-away-clouds': 'Вранішня-Зоря-Розганяє-Хмари',
    'not-so-quick': 'Не-Такий-Прудкий',
    'only-he-stands-there': 'Лише-Він-Там-Стоїть',
    'scales-shine-brightly': 'Луска-Яскраво-Сяє',
    'shallow-thoughts': 'Дрібні-Думки', 'smart-snake': 'Розумна-Змія',
    'smokeskin-killer': 'Димошкір-Убивця', 'snail-tail': 'Слимачий-Хвіст',
    'stream-murk': 'Каламуть-Потоку', 'swims-in-swells': 'Плаває-У-Хвилях',
    'tern-feather': 'Перо-Крячка', 'threads-in-groves': 'Снує-У-Гаях',
    'tongue-toad': 'Язиката-Жаба', 'twice-bitten': 'Двічі-Вкушений',
    'traveling-new-woman': 'Мандрівна-Нова-Жінка',
    'travelling-new-woman': 'Мандрівна-Нова-Жінка',
    'wind-in-his-hair': 'Вітер-У-Волоссі',
})

# діграфи (спершу), потім поодинокі
DIGRAPHS = [('tch', 'ч'), ('sch', 'ш'), ('sh', 'ш'), ('ch', 'ч'), ('th', 'т'),
            ('ph', 'ф'), ('ck', 'к'), ('kh', 'х'), ('gh', 'г'), ('zh', 'ж'),
            ('ll', 'лл'), ('oo', 'у'), ('ee', 'і'), ('ou', 'у'), ('ay', 'ей'),
            ('ai', 'ай'), ('ei', 'ей'), ('oi', 'ой'), ('ia', 'ія'), ('io', 'іо'),
            ('qu', 'кв'), ('x', 'кс'), ('yu', 'ю'), ('ya', 'я'), ('yo', 'йо'),
            # ye -> є, як і решта йотованих. Без нього Yessur давало «Іессур»,
            # а Dirvanye - «Дірвание»
            ('ye', 'є'),
            # ie -> іє: «Іеніт» проти «Ієніт» в активаторах. Зачіпає 117 імен,
            # і скрізь на краще - українською після «і» пишемо «є», не «е»
            ('ie', 'іє')]
# g -> г, а не ґ: див. CONVENTIONS §4. Доти двигун давав ґ, і кожен
# `loc.py rebuild` мовчки відкочував усталені руками імена назад у
# «Ґотрен» - правило жило в документі, а не в інструменті.
SINGLE = {'a': 'а', 'b': 'б', 'c': 'к', 'd': 'д', 'e': 'е', 'f': 'ф', 'g': 'г',
          'h': 'г', 'i': 'і', 'j': 'дж', 'k': 'к', 'l': 'л', 'm': 'м', 'n': 'н',
          'o': 'о', 'p': 'п', 'q': 'к', 'r': 'р', 's': 'с', 't': 'т', 'u': 'у',
          'v': 'в', 'w': 'в', 'x': 'кс', 'y': 'и', 'z': 'з', "'": "'", '-': '-'}


def translit_word(w):
    """Фонетична транслітерація: звучання лишаємо як в оригіналі, нічого не
    скорочуємо. Єдиний контекстний випадок — м'яка c перед e/i/y (Lucius->Луціус,
    Cyrodiil->Сиродііл), інакше c->к (Vivec->Вівек, Caius->Кайус)."""
    s = w.lower()
    # римська традиція для закінчення -ius (у грі -ius = імперські імена):
    # -cius/-tius -> -цій (палаталізація: Lucius->Луцій, Gratius->Грацій),
    # інше -ius -> -ій (Tullius->Туллій, Antonius->Антоній, Flavius->Флавій).
    tail = ''
    if len(s) > 4 and (s.endswith('cius') or s.endswith('tius')):
        tail = 'цій'
        s = s[:-4]
    elif len(s) > 4 and s.endswith('ius'):
        tail = 'ій'
        s = s[:-3]
    out = []
    i = 0
    while i < len(s):
        for dg, rep in DIGRAPHS:
            if s.startswith(dg, i):
                out.append(rep)
                i += len(dg)
                break
        else:
            ch = s[i]
            if ch == 'c':
                nxt = s[i + 1] if i + 1 < len(s) else ''
                # класична традиція: c перед e/i/y -> ц (Цезар, Цицерон, Луцій),
                # інакше -> к (Vivec->Вівек, Caius->Кай, Cosades->Косадес)
                out.append('ц' if nxt in ('e', 'i', 'y') else 'к')
            elif ch == 'y':
                # кінцеве -y -> -і; на початку слова теж і, бо українське слово
                # з «и» не починається (Yngling -> Інґлінґ, а не Инґлінґ)
                out.append('і' if i in (0, len(s) - 1) else 'и')
            else:
                out.append(SINGLE.get(ch, ch))
            i += 1
    res = ''.join(out) + tail
    res = res[:1].upper() + res[1:]
    res = re.sub(r"-(.)", lambda m: '-' + m.group(1).upper(), res)   # Тімсар-Дадісун
    # велика літера після апострофа для префіксів (Дж'Кара, Дро'Сахар)...
    res = re.sub(r"'(.)", lambda m: "'" + m.group(1).upper(), res)
    # ...окрім присвійного 's у кінці слова (Андреті'с, не Андреті'С)
    if res.endswith("'С"):
        res = res[:-1] + 'с'
    return res


def convert(name):
    if '<' in name or '>' in name:       # службові плейсхолдери лишаємо як є
        return name
    parts = re.split(r'(\s+)', name)     # зберегти пробіли
    out = []
    for p in parts:
        if not p.strip():
            out.append(p)
            continue
        low = p.lower()
        if low in WORDS:
            out.append(WORDS[low])
        elif low.startswith('gro-') or low.startswith('gra-'):
            pref = WORDS[low[:3]]
            out.append(pref + '-' + translit_word(p[4:]))
        else:
            out.append(translit_word(p))
    return re.sub(r'\s+', ' ', ' '.join(w for w in out if w != '')).strip()


def main():
    apply = '--apply' in sys.argv
    names = list(json.load(open(os.path.join(HERE, 'npc.json'), encoding='utf-8')))
    ov = {}
    op = os.path.join(HERE, 'npc_overrides.json')
    if os.path.isfile(op):
        ov = json.load(open(op, encoding='utf-8'))
        ov.pop('_comment', None)
    out = {}
    for n in names:
        out[n] = ov.get(n) or convert(n)
    if apply:
        with open(os.path.join(HERE, 'uk_npc.json'), 'w', encoding='utf-8') as f:
            json.dump({k: out[k] for k in sorted(out)}, f, ensure_ascii=False, indent=1)
        print('ЗАПИСАНО uk_npc.json:', len(out))
    else:
        import random
        random.seed(1)
        for n in random.sample(names, 40):
            print('  %-28s -> %s' % (n, out[n]))


if __name__ == '__main__':
    main()
