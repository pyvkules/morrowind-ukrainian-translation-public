...
def tokens(text):
    """Набір підстановок, без огляду на регістр.

    `%Class` і `%class` рушій підставляє однаково, тож різниця в регістрі
    тут — не різниця. Важливо лише, щоб набір збігався: підстановка, якої
    в старому перекладі нема, лишила б у грі дірку.
    """
    return sorted(t.lower() for t in inherit.TOKEN.findall(text))
